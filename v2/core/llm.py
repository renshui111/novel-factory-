...
def _get_ollama_urls() -> list:
    urls = []
    from .config import load_config
    cfg = load_config()
    base = cfg.get("local_llm", {}).get("base_url", "http://localhost:11434/v1")
    m = re.match(r'(https?://[^/]+)', base)
    if m:
        api = m.group(1).rstrip('/')
        urls.append(f"{api}/api/tags")
    configured_port = api.split(':')[-1] if ':' in api else ''
    for port in ['11434', '11435', '11436', '8080', '11433']:
        if port != configured_port:
            urls.append(f"http://localhost:{port}/api/tags")
    urls.append("http://localhost:11434/api/tags")
    seen = set()
    unique = []
    for u in urls:
        if u not in seen:
